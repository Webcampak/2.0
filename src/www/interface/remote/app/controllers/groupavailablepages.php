...
*/
/**
 * @class Groupavailablepages
 * A simple application controller extension
 */
class Groupavailablepages extends ApplicationController {
    /**
     * view
     * Retrieves rows from database.
     */
	public function view() {
		Log::debug("Log: /remote/app/controllers/groupavailablepages.php - view() function");
		$userAuth = new Authorizations();
		if ($userAuth->isAllowedToAccessPage("auth.config.admin.permissions") == true) {
			$res = new Response();
			$res->success = true;
			$res->message = "Loaded data";
			$res->data = GroupAvailablePage::all();
			return $res->to_json();
		} else {
	...
	public function create() {
		Log::debug("Log: /remote/app/controllers/groupavailablepages.php - create() function");
		$userAuth = new Authorizations();
		if ($userAuth->isAllowedToAccessPage("auth.config.admin.permissions") == true) {    	
			$res = new Response();
			$rec = GroupAvailablePage::create($this->params);
			if ($rec) {
				$res->success = true;
	...
	public function update() {
		Log::debug("Log: /remote/app/controllers/groupavailablepages.php - update() function");
		$userAuth = new Authorizations();
		if ($userAuth->isAllowedToAccessPage("auth.config.admin.permissions") == true) {       	
			$res = new Response();
			$rec = GroupAvailablePage::update($this->id, $this->params);
			if ($rec) {
				$res->data = $rec->to_hash();
				$res->success = true;
	...
	public function destroy() {
		Log::debug("Log: /remote/app/controllers/groupavailablepages.php - destroy() function");
		$userAuth = new Authorizations();
		if ($userAuth->isAllowedToAccessPage("auth.config.admin.permissions") == true) {     	
			$res = new Response();
			if (GroupAvailablePage::destroy($this->id)) {
				$res->success = true;
