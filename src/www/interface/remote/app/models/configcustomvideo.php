...
*/
/**
 * @class Configcustomvideo
 */
class Configcustomvideo  { //extends Model {
	public $id, $attributes;	

	static function all() {
		Log::debug("Log: /remote/lib/models/configcustomvideo.php - all() function");
		...
		if ($sourceid > 0 && $userAuth->isAllowedToAccessSource($sourceid) == true) {
			...
			$configSettings = $configObj->getSettings("config-source" . $sourceid . "-videocustom.cfg", $sourceid, NULL);
			return $configSettings;
		}
	}

	static function update($params) {
		Log::debug("Log: /remote/lib/models/configcustomvideo.php - update() function");
		$userAuth = new Authorizations();
		if (isset($_REQUEST["sourceid"])) {$sourceid = (int) strip_tags($_REQUEST["sourceid"]);} 
		if ($sourceid > 0 && $userAuth->isAllowedToAccessSource($sourceid) == true && $userAuth->isAllowedToEditSource($sourceid) == true) {
			...
			$configObj = new ConfigObj();
			$configSettingsInit = $configObj->getSettings("config-source" . $sourceid . "-videocustom.cfg", $sourceid, NULL);
			...
			Log::debug("Log: /remote/lib/models/configcustomvideo.php - update() - User not allowed to edit source");	
			return false;
		}
	}
}
