...
*/
/**
 * @class Cloudconfigpicture
 */
class Cloudconfigpicture  { //extends Model {
	public $id, $attributes;	

	static function all() {
		Log::debug("Log: /remote/lib/models/cloudconfigpicture.php - all() function");
		...
		if ($sourceid > 0 && $userAuth->isAllowedToAccessSource($sourceid) == true) {
			...
			$configSettings = $configObj->getSettings("config-source" . $sourceid . ".cfg", $sourceid, unserialize(CFGCLOUD_PICTURES));
			return $configSettings;
		}
	}

	static function update($params) {
		Log::debug("Log: /remote/lib/models/cloudconfigpicture.php - update() function");
		$userAuth = new Authorizations();
		if (isset($_REQUEST["sourceid"])) {$sourceid = (int) strip_tags($_REQUEST["sourceid"]);} 
		if ($sourceid > 0 && $userAuth->isAllowedToAccessSource($sourceid) == true && $userAuth->isAllowedToEditSource($sourceid) == true) {
			...
			$configObj = new ConfigObj();
			$configSettingsInit = $configObj->getSettings("config-source" . $sourceid . ".cfg", $sourceid, unserialize(CFGCLOUD_PICTURES));
			$configSettingsNew = array();
			$configSettingsTmp = array();			
			foreach($params as $key=>$value) {
				$value = strval($value);
				if (isset($configSettingsInit[$key]) && $configSettingsInit[$key] != $value) {
					if (in_array($key, unserialize(CFGCLOUD_PICTURES))) { // we look into this list to find the current key, if found, key is added, otherwise key is discarded
			...
			if (isset($configSettingsNew['cfgcroncapturevalue']) || isset($configSettingsNew['cfgcroncaptureinterval'])) {
				Log::debug("Log: /remote/lib/models/cloudconfigpicture.php - update() - Update global cron");					
			...
			if (isset($configSettingsNew['cfglocalftppass'])) {
				Log::debug("Log: /remote/lib/models/cloudconfigpicture.php - update() - Update global FTP configuration");					
			...
			Log::debug("Log: /remote/lib/models/cloudconfigpicture.php - update() - User not allowed to edit source");	
			return false;
		}
	}
}
