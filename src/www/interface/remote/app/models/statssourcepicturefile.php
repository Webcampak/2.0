 ...
 */
class Statssourcepicturefile  { 
	public $id, $attributes;	

	static function all() {
		Log::debug("Log: /remote/lib/models/statssourcepicturefile.php - all() function");
		$statsvalues = array();
		$statsdays = array();
		...
		Log::debug("Log: /remote/lib/models/statssourcepicturefile.php - all() - sourceid: " . $sourceid );
		if ($sourceid == 0) {
			//If no source is defined by Sencha, send a request to get allowed sources, select the ID with smallest weight.
			Log::debug("Log: /remote/lib/models/statssourcepicturefile.php - all() - No sources specified by Sencha, looking into database");
			$allowedsources = $userAuth->userAllowedSources();
			$sourceid = $allowedsources[0]['sourceid'];
			Log::debug("Log: /remote/lib/models/statssourcepicturefile.php - all() - No sources specified by Sencha, using source: " . $sourceid);				
		}	
		if ($sourceid > 0) {
			...
			foreach ($allowedSourcesArr as $allowedSources) {
				Log::debug("Log: /remote/lib/models/statssourcepicturefile.php - all() - Source: " . $allowedSources['sourceid']);
				...
				foreach($statsvalues as $key=>$value) {
					foreach($value as $subkey=>$subvalue) {
						if ($subvalue['date'] == $day) {
							...
							$tmpstatsseries['graphdata'] = $subvalue['nbpics'];
						}
					}
				}
