...
require(dirname(__FILE__).'/database/users.php');
require(dirname(__FILE__).'/database/sources.php');	

require(dirname(__FILE__).'/authorization/authorizations.php');	
...
define ('DIRECTORY_CACHE', dirname(__FILE__).'/timthumb/cache/');		// cache directory
define ('DIRECTORY_TEMP', dirname(__FILE__).'/timthumb/tmp/');		// temp directory
...
if ($userAuth->isAllowedToAccessSource($sourceid)) {
	$sourceallowed = 1;
	Log::debug("Log: /remote/lib/timthumb.php - User allowed to access source: " . $sourceid);
} else {
	Log::debug("Log: /remote/lib/timthumb.php - User not allowed to access source: " . $sourceid);				
	exit();	
}
...
$src = CFGDIR_WEBSOURCESDIR . "source" . $sourceid . "/" . $src;

Log::access("Timthumb: " . $src);

//$src = CFGDIR_SOURCESDIR . "source" . $sourceid . "/" . $src;
